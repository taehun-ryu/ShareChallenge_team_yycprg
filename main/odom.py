...
def target_odo_move():
    global response
    global sig
    global target_x, target_y, now_x, now_y
    if data != None:
        commend = data.decode()
        ...
        if len(m) == 5:
            ...
            if commend == 'fl' and (target_x-now_x)<=0:

                if sig != 5:
                    go(20,5)
                    sig = 5
            elif commend == 'fr' and (target_x-now_x)<0:
                if sig != 6:
                    go(5,20)
                    sig = 6
            elif commend == 'fb' and (target_x-now_x)<0:
                if sig != 7:
                    go(-20,-20)
                    sig = 7
